...
\`{ "name": "Persona Name", "html": "..." }\`
  ...
  const handleExpandPrototype = useCallback(async () => {
      const currentSession = sessions[currentSessionIndex];
      if (!currentSession || focusedArtifactIndex === null) return;
      const currentArtifact = currentSession.artifacts[focusedArtifactIndex];
      ...
      const newSession: Session = {
          id: sessionId,
          prompt: `Expand: ${currentSession.prompt}`,
          timestamp: baseTime,
      ...
      try {
          ...
          const prompt = `
You are Flash UI. The user wants to EXPAND and REFINE a generated UI component into a full, cohesive, responsive prototype.

**ORIGINAL PROMPT:** "${currentSession.prompt}"
**ART DIRECTION / STYLE:** "${currentArtifact.styleName}"

**EXISTING HTML (Use this as your foundation and style guide):**
\`\`\`html
${currentArtifact.html}
\`\`\`

**YOUR TASK:**
1. Take the existing HTML and push the layout design much further into a full-page, highly usable, responsive prototype (looks great on mobile to desktop).
2. Add cohesive, connected content (e.g., tabs changing sections, linking nav buttons, section titles, contextual copy that makes sense for "${currentSession.prompt}").
3. Preserve the exact look and feel, CSS variables, and atmosphere of the existing design, but refine and consolidate the layout so it feels like a real, functioning product.
4. Add polished hover states, subtle CSS animations, and functional-looking (but purely frontend) interactivity using inline HTML/CSS/JS if necessary. Give it a proper structure: Header, Main Content, Sidebar/Grid, Footer, etc.

Return ONLY RAW HTML. No markdown fences. Ensure it begins with <style> or structured HTML and contains everything needed.
          `.trim();
                  ...
                  setSessions(prev => prev.map(sess => 
                      sess.id === sessionId ? {
                          ...
                          artifacts: sess.artifacts.map(art => 
                              art.id === placeholderArtifacts[0].id ? { ...art, html: accumulatedHtml } : art
                          )
                      } : sess
                  ));
              }
          ...
          setSessions(prev => prev.map(sess => 
              sess.id === sessionId ? {
                  ...sess,
                  artifacts: sess.artifacts.map(art => 
                      art.id === placeholderArtifacts[0].id ? { ...art, html: finalHtml, status: finalHtml ? 'complete' : 'error' } : art
                  )
              } : sess
          ));

      } catch (e: any) {
          console.error('Error expanding prototype:', e);
          setSessions(prev => prev.map(sess => 
              sess.id === sessionId ? {
                  ...sess,
                  artifacts: sess.artifacts.map(art => 
                      art.id === placeholderArtifacts[0].id ? { ...art, html: `<div style="color: #ff6b6b; padding: 20px;">Error: ${e.message}</div>`, status: 'error' } : art
                  )
              } : sess
          ));
      } finally {
  ...
  const handleExportHTML = () => {
      ...
      if (currentSession && focusedArtifactIndex !== null) {
          const artifact = currentSession.artifacts[focusedArtifactIndex];
          let htmlContent = artifact.html;
          
          const lowerHtml = htmlContent.toLowerCase();
          // Ensure it's a complete HTML document if it isn't already
...
<head>
    ...
    <title>${artifact.styleName} - Design Export</title>
</head>
...
${htmlContent}
          ...
          a.href = url;
          a.download = `${artifact.styleName.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_design.html`;
        ...
        <div className="immersive-app">
            ...
            <div className={`stage-container ${focusedArtifactIndex !== null ? 'mode-focus' : 'mode-split'}`}>
                 <div className={`empty-state ${hasStarted ? 'fade-out' : ''}`}>
                     <div className="empty-content">
                         ...
                         <button className="surprise-button" onClick={handleSurpriseMe} disabled={isLoading} title={isLoading ? "Wait for current generation to finish" : ""}>
            ...
            <div className={`action-bar ${focusedArtifactIndex !== null ? 'visible' : ''}`}>
                 ...
                 <div className="action-buttons">
                    ...
                    <button onClick={handleGenerateVariations} disabled={isLoading} aria-label="Generate design variations" title={isLoading ? "Wait for current generation to finish" : "Generate design variations"}>
                        <SparklesIcon /> Variations
                    </button>
                    <button onClick={handleExpandPrototype} disabled={isLoading} aria-label="Expand to full prototype" title={isLoading ? "Wait for current generation to finish" : "Expand to full prototype"}>
                        <ExpandIcon /> Expand
                    </button>
            ...
            <div className="floating-input-container">
                <div className={`input-wrapper ${isLoading ? 'loading' : ''}`}>
                    ...
                    <button className="send-button" onClick={() => handleSendMessage()} disabled={isLoading || !inputValue.trim()} aria-label="Send prompt" title={isLoading ? "Wait for current generation to finish" : (!inputValue.trim() ? "Enter a prompt first" : "Send prompt")}>
                        <ArrowUpIcon />
                    </button>
                </div>
            </div>
